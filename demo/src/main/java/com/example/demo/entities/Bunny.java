...
public class Bunny extends Animal {
    ...
    private Vector2d flee(List<Wolf> wolves) {
        ...
        for (Wolf wolf : wolves) {
            double dx = x - wolf.getX();
            double dy = y - wolf.getY();
    ...
    private Vector2d seekGrass(List<Grass> grassList) {
        ...
        for (Grass grass : grassList) {
            double dx = x - grass.getX();
            double dy = y - grass.getY();
            double dist = Math.sqrt(dx * dx + dy * dy);
    ...
    private Vector2d cohesion(List<Bunny> bunnies) {
        ...
        for (Bunny bunny : bunnies) {
            center.add(new Vector2d(bunny.getX(), bunny.getY()));
        }
        center.div(bunnies.size());
        
        Vector2d desired = new Vector2d(center.x - x, center.y - y);
        desired.normalize();
        desired.mult(speed);
